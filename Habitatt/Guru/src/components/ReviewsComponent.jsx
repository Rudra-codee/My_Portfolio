...
export default function Reviews() {
  ...
  // Card animations
  const [cardAnims, _] = useSpring(() => ({
    from: reviews.map(() => ({ opacity: 0, y: 30 })),
    to: reviews.map(() => ({ opacity: 1, y: 0 })),
    config: { tension: 200, friction: 20 },
    delay: (i) => i * 150
  }));
  ...
  return (
    <section className="reviews-section">
      <animated.div style={containerAnim} className="reviews-container">
        ...
        <div className="reviews-grid">
          {reviews.map((review, index) => (
            <animated.div 
              ...
              style={{
                opacity: cardAnims.opacity.to(o => o[index]),
                y: cardAnims.y.to(y => y[index])
              }}
            >
